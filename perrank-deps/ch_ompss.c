...
/**
 * TODO: What is the lower bound for a circular deadlock? (0 waits for 1 waits
 * for 2 waits for 0) Example: Execution order on 1 is reversed: 0 waits for
 * 1/2/3, 1 waits for 3/2/0, 2 waits for 0/1/3, 3 waits for 0/1/2 OR 0 waits for
 * 1/2/3/4, 1 waits for 0/2/3/4, 2 waits for 4/3/2/0, 3 waits for 0/1/2/4, 4
 * waits for 0/1/2/3 OR 0 waits for 1/2/3/4/5, 1 waits for 0/2/3/4/5, 2 waits
 * for 5/4/3/2/0, 3 waits for 0/1/2/4/5, 4 waits for 0/1/2/3/5, 5 waits for
 * 0/1/2/3/5
 *
 * NOTE: circular dependencies may happen if at least one of the inner ranks
 *       (1 or 2, not 0 or 3) reverse their order
 * HYPOTHESIS: we need at least (p-(p/2)) (ceil(0.5p)) threads to avoid deadlock
 * from reversal Generalization to some ordered graph traversal problem?
 */
...
#pragma omp task in(B)out(A[k][i]) firstprivate(k, i) no_copy_deps
          ...
          for (int dst = 0; dst < np; dst++) {
            ...
            // recv_cnt, (nt-(k+1)));
            // NOTE: we have to wait for all of the above tasks using
            // comm_sentinel
            //       dependency iterators might help here
#pragma omp task no_copy_deps firstprivate(k, dst) out(                        \
    { C[recv_blocks[it]], it = 0; recv_cnt })                                  \
    in(                                                                        \
        { A[k][send_blocks[it]], it = 0; send_cnt }) untied
            {
              START_TIMING(TIME_COMM);
              int nreqs = 0;
    ...
    no_copy_deps
                  {
                    ...
                    EXTRAE_EXIT(EVENT_GEMM);
                           ...
                           block_rank[k * nt + j] == mype) {
#pragma omp task in(A[k][j], C[i])out(A[j][i]) firstprivate(k, j, i)           \
    no_copy_deps
                  {
                    EXTRAE_ENTER(EVENT_GEMM);
                    START_TIMING(TIME_GEMM);
                    ...
                    EXTRAE_EXIT(EVENT_GEMM);
                           ...
                           block_rank[k * nt + j] != mype) {
                  // TODO: the content of C[j] may be overwritten but we cannot
