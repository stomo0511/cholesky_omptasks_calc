...
double *alpha, const void *a, int *lda, void *b, int *ldb, double *beta, void
*c, int *ldc);
...
static void convert_to_blocks(const int ts, const int DIM, const int N,
                              double Alin[N][N], double *A[DIM][DIM]) {
  for (int i = 0; i < DIM; i++)
    for (int j = 0; j < DIM; j++) {
      gather_block(N, ts, &Alin[i * ts][j * ts], A[i][j]);
    }
}
static void convert_to_blocks_for_onesided(const int ts, const int DIM,
                                           const int N, double Alin[N][N],
                                           double *A) {
  for (int i = 0; i < DIM; i++)
    for (int j = 0; j < DIM; j++) {
...
static void convert_to_linear(const int ts, const int DIM, const int N,
                              double *A[DIM][DIM], double Alin[N][N]) {
  for (int i = 0; i < DIM; i++)
    for (int j = 0; j < DIM; j++) {
