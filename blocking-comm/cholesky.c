        ...
        for (int k = 0; k < nt; k++) {
          if (block_rank[k * nt + k] == mype) {
            ...
            if (np != 1) {
              ...
              //#pragma omp task depend(in: A[k][k], comm_sentinel)
              // firstprivate(k)
              {
                ...
                MPI_Waitall(reqs, nreqs);
                free(reqs);
#else
                for (int dst = 0; dst < np; dst++) {
                  if (send_flags[dst] && dst != mype) {
                    MPI_Send(A[k][k], ts * ts, MPI_DOUBLE, dst, k * nt + k,
                             MPI_COMM_WORLD);
                  }
                }
#endif
            ...
            // use comm_sentinel to make sure this task runs before the
            // communication tasks below
            //#pragma omp task depend(out: B) depend(in:comm_sentinel)
            // firstprivate(k)
            {
              START_TIMING(TIME_COMM);
              int recv_flag = 0;
              ...
              if (recv_flag) {
                ...
                // printf("Receiving potrf block from %d in k=%d\n",
                   block_rank[k*nt+k], k);
                   MPI_Waitall(&recv_req, 1, MPI_STATUSES_IGNORE);
#else
                MPI_Recv(B, ts * ts, MPI_DOUBLE, block_rank[k * nt + k],
                         k * nt + k, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
#endif
              }
              END_TIMING(TIME_COMM);
            }
...
#pragma omp task depend(in                                                     \
                        : A[k][k], comm_sentinel) depend(out                   \
                                                         : A[k][i])            \
    firstprivate(k, i)
                {
...
#pragma omp taskwait
          //#pragma omp task depend(inout : comm_sentinel) firstprivate(k)
          // shared(A)
          {
            START_TIMING(TIME_COMM);
            char send_flags[np];
            reset_send_flags(send_flags);
            int nreqs = 0;
#if defined(IRECV) || defined(ISEND)
            ...
            for (int i = k + 1; i < nt; i++) {
              if (block_rank[k * nt + i] == mype && np != 1) {
                ...
                for (int dst = 0; dst < np; dst++) {
                  if (send_flags[dst] && dst != mype) {
                    ...
                    reqs[nreqs++] = send_req;
#else
                    MPI_Send(A[k][i], ts * ts, MPI_DOUBLE, dst, k * nt + i,
                             MPI_COMM_WORLD);
#endif
                  }
                }
              ...
              if (block_rank[k * nt + i] != mype) {
                ...
                if (recv_flag) {
                  ...
                  reqs[nreqs++] = recv_req;
#else
                  MPI_Recv(C[i], ts * ts, MPI_DOUBLE, block_rank[k * nt + i],
                           k * nt + i, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
#endif
                }
              }
            }
...
#if defined(IRECV) || defined(ISEND)
            MPI_Waitall(reqs, nreqs, MPI_STATUSES_IGNORE);
            free(reqs);
#endif
            END_TIMING(TIME_COMM);
          }
    ...
    firstprivate(k, j, i)
                  {
                    ...
                    END_TIMING(TIME_GEMM);
                           ...
                           block_rank[k * nt + j] == mype) {
#pragma omp task depend(in                                                     \
                        : A[k][j], comm_sentinel) depend(out                   \
                                                         : A[j][i])            \
    firstprivate(k, j, i)
                  {
                    START_TIMING(TIME_GEMM);
                    ...
                    END_TIMING(TIME_GEMM);
                           ...
                           block_rank[k * nt + j] != mype) {
#pragma omp task depend(in                                                     \
                        : A[k][i], comm_sentinel) depend(out                   \
                                                         : A[j][i])            \
    firstprivate(k, j, i)
                  {
                    START_TIMING(TIME_GEMM);
