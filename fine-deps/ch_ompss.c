...
#pragma omp task depend(in                                                     \
    ...
    depend(in                                                                  \
           : comm_round_sentinel) untied
            {
              START_TIMING(TIME_COMM);
              MPI_Request reqs[np];
              int nreqs = 0;
              // printf("[%d:%d:%d] Sending k=%d block (tag %d)\n", mype,
              // omp_get_thread_num(), depth, k, k*nt+k);
    ...
    untied
              {
                ...
                // mype, omp_get_thread_num(), depth, k, block_rank[k*nt+k],
                // k*nt+k);
                START_TIMING(TIME_COMM);
                depth++;
                ...
                depth--;
                END_TIMING(TIME_COMM);
                // printf("[%d:%d:%d] Done Receiving k=%d block from %d (tag
                // %d)\n", mype, omp_get_thread_num(), depth, k,
                // block_rank[k*nt+k], k*nt+k);
              }
              recv_flag = 0;
            }
          }
...
#pragma omp task depend(in                                                     \
    ...
    depend(in                                                                  \
           : comm_round_sentinel) untied
                  {
                    // printf("[%d:%d:%d] Sending k=%d i=%d block to %d (tag
                    // %d)\n", mype, omp_get_thread_num(), depth, k, i, dst,
                    // k*nt+i);
                    START_TIMING(TIME_COMM);
                    depth++;
                    MPI_Request send_req;
                    ...
                    depth--;
                    END_TIMING(TIME_COMM);
                    // printf("[%d:%d:%d] Done Sending k=%d i=%d block to %d
                    // (tag %d)\n", mype, omp_get_thread_num(), depth, k, i, dst,
                    // k*nt+i);
                  }
                }
...
#pragma omp task depend(out                                                    \
    ...
    depend(in                                                                  \
           : comm_round_sentinel) untied
                {
                  // printf("[%d:%d:%d] Receiving k=%d i=%d block from %d (tag
                  // %d)\n", mype, omp_get_thread_num(), depth, k, i,
                  // block_rank[k*nt+i], k*nt+i);
                  START_TIMING(TIME_COMM);
                  depth++;
                  MPI_Request recv_req;
                  ...
                  depth--;
                  END_TIMING(TIME_COMM);
                  // printf("[%d:%d:%d] Done Receiving k=%d i=%d block from %d
                  // (tag %d)\n", mype, omp_get_thread_num(), depth, k, i,
                  // block_rank[k*nt+i], k*nt+i);
                }
                recv_flag = 0;
