...
static int get_thread_num() {
  ...
  if (thread_num == -1)
    thread_num = my_next_id();
  return thread_num;
}
static int get_num_threads() { return ID; }
...
#define PRINT_TIMINGS()                                                        \
  do {                                                                         \
    ...
    printf("[%d] "                                                             \
           "potrf:%f:trsm:%f:gemm:%f:syrk:%f:comm:%f:create:%f:non-calc:%f:"   \
           "total:%f:wall:%f\n",                                               \
           mype, acc_timings.ts[TIME_POTRF], acc_timings.ts[TIME_TRSM],        \
           acc_timings.ts[TIME_GEMM], acc_timings.ts[TIME_SYRK],               \
           acc_timings.ts[TIME_COMM], acc_timings.ts[TIME_CREATE],             \
           acc_timings.ts[TIME_TOTAL] * NUM_THREADS -                          \
               acc_timings.ts[TIME_POTRF] - acc_timings.ts[TIME_TRSM] -        \
               acc_timings.ts[TIME_GEMM] - acc_timings.ts[TIME_SYRK],          \
           acc_timings.ts[TIME_TOTAL],                                         \
           acc_timings.ts[TIME_TOTAL] * NUM_THREADS);                          \
    if (mype == 0)                                                             \
      MPI_Reduce(MPI_IN_PLACE, acc_timings.ts, TIME_CNT, MPI_DOUBLE, MPI_SUM,  \
                 0, MPI_COMM_WORLD);                                           \
    else                                                                       \
      MPI_Reduce(acc_timings.ts, NULL, TIME_CNT, MPI_DOUBLE, MPI_SUM, 0,       \
                 MPI_COMM_WORLD);                                              \
    if (mype == 0)                                                             \
      printf("[total] "                                                        \
             "potrf:%f:trsm:%f:gemm:%f:syrk:%f:comm:%f:create:%f:non-calc:%f:" \
...
#define START_TIMING(timer)                                                    \
  double __ts_##timer = timestamp();                                           \
  int __timer = timer;
#define END_TIMING(timer)                                                      \
  __timing[THREAD_NUM].ts[timer] += timestamp() - __ts_##timer

static double timestamp() { return MPI_Wtime(); }

#define wait(req) wait_impl(req, &__timing[THREAD_NUM].ts[__timer])
#define waitall(req, nreq)                                                     \
  waitall_impl(req, nreq, &__timing[THREAD_NUM].ts[__timer])
